...
class AstVisitor(SolidityVisitor):
    ...
    def visitSourceUnit(self, ctx):
        return Node(
            ctx=ctx, type="SourceUnit", children=self.visit(ctx.children[:-1])
        )  # skip EOF
    ...
    def visitEnumValue(self, ctx):
        return Node(ctx=ctx, type="EnumValue", name=ctx.identifier().getText())
    ...
    def visitConstructorDefinition(
    ...
    def visitFunctionDefinition(self, ctx: SolidityParser.ConstructorDefinitionContext):
        ...
        parameters = self.visit(ctx.parameterList())
        returnParameters = (
            self.visit(ctx.returnParameters()) if ctx.returnParameters() else []
        )
        block = self.visit(ctx.block()) if ctx.block() else []
        modifiers = [self.visit(i) for i in ctx.modifierList().modifierInvocation()]
    ...
    def visitParameterList(self, ctx: SolidityParser.ParameterListContext):
        ...
        return Node(ctx=ctx, type="ParameterList", parameters=parameters)

    def visitParameter(self, ctx: SolidityParser.ParameterContext):

        storageLocation = (
            ctx.storageLocation().getText() if ctx.storageLocation() else None
        )
    ...
    def visitFunctionTypeName(self, ctx):
        parameterTypes = [
            self.visit(p)
            for p in ctx.functionTypeParameterList(0).functionTypeParameter()
        ]
        returnTypes = []
        ...
        if ctx.functionTypeParameterList(1):
            returnTypes = [
                self.visit(p)
                for p in ctx.functionTypeParameterList(1).functionTypeParameter()
            ]
    ...
    def visitEmitStatement(self, ctx):
        return Node(
            ctx=ctx, type="EmitStatement", eventCall=self.visit(ctx.getChild(1))
        )
    ...
    def visitUserDefinedTypeName(self, ctx):
        return Node(ctx=ctx, type="UserDefinedTypeName", namePath=ctx.getText())

    def visitElementaryTypeName(self, ctx):
        return Node(ctx=ctx, type="ElementaryTypeName", name=ctx.getText())

    def visitBlock(self, ctx):
        return Node(ctx=ctx, type="Block", statements=self.visit(ctx.statement()))

    def visitExpressionStatement(self, ctx):
        return Node(
            ctx=ctx, type="ExpressionStatement", expression=self.visit(ctx.expression())
        )
    ...
    def visitExpression(self, ctx):
        ...
        if children_length == 1:
            ...
            if op == "new":
                return Node(
                    ctx=ctx, type="NewExpression", typeName=self.visit(ctx.typeName())
                )
            ...
            if ctx.getChild(1).getText() == "(" and ctx.getChild(3).getText() == ")":
                ...
                if ctxArgs.expressionList():
                    args = [
                        self.visit(a) for a in ctxArgs.expressionList().expression()
                    ]
                elif ctxArgs.nameValueList():
                    for nameValue in ctxArgs.nameValueList().nameValue():
                        args.append(self.visit(nameValue.expression()))
    ...
    def visitForStatement(self, ctx):
        conditionExpression = (
            self.visit(ctx.expressionStatement()) if ctx.expressionStatement() else None
        )
    ...
    def visitPrimaryExpression(self, ctx):
        ...
        if ctx.HexLiteral():
            return Node(ctx=ctx, type="HexLiteral", value=ctx.HexLiteral().getText())

        if ctx.StringLiteral():
            text = ctx.getText()

            return Node(ctx=ctx, type="StringLiteral", value=text[1 : len(text) - 1])
        ...
        if (
            ...
            if node.type == "Identifier":
                node = Node(ctx=ctx, type="UserDefinedTypeName", namePath=node.name)
            else:
                node = Node(
                    ctx=ctx, type="ElementaryTypeName", name=ctx.getChild(0).getText()
                )

            return Node(ctx=ctx, type="ArrayTypeName", baseTypeName=node, length=None)

        return self.visit(ctx.getChild(0))

    def visitIdentifier(self, ctx):
        return Node(ctx=ctx, type="Identifier", name=ctx.getText())

    def visitTupleExpression(self, ctx):
        children = ctx.children[1:-1]
        components = [
            None if e is None else self.visit(e)
            for e in self._mapCommasToNulls(children)
        ]
    ...
    def visitAssemblyBlock(self, ctx):
        ...
        return Node(ctx=ctx, type="AssemblyBlock", operations=operations)
    ...
    def visitAssemblyLiteral(self, ctx):
        ...
        if ctx.DecimalNumber():
            return Node(ctx=ctx, type="DecimalNumber", value=ctx.getText())

        if ctx.HexNumber():
            return Node(ctx=ctx, type="HexNumber", value=ctx.getText())

        if ctx.HexLiteral():
            return Node(ctx=ctx, type="HexLiteral", value=ctx.getText())
    ...
    def visitLabelDefinition(self, ctx):
        return Node(ctx=ctx, type="LabelDefinition", name=ctx.identifier().getText())

    def visitAssemblyStackAssignment(self, ctx):
        return Node(
            ctx=ctx, type="AssemblyStackAssignment", name=ctx.identifier().getText()
        )
    ...
    def visitUserDefinedTypename(self, ctx):
        return Node(ctx=ctx, type="UserDefinedTypename", name=ctx.getText())
